...
PhoneBook::~PhoneBook() {
	...
	for (int i = 0; i < HASH_MAXSIZE; i++)
	{
		...
		else
		{
			...
			*/
			for (int count=0;count<lengthnumberHash[i];count++)
			{
				PhoneNumber* deletenext = deletetemp->next;
				deletetemp->next = nullptr;
				delete deletetemp;
	...
	for (int i = 0; i < HASH_MAXSIZE; i++)
	{
		...
		{
			deletetemp = deletetemp->next;//value的下一个指针
			for (int count=0;count<lengthnameHash[i];count++)
			{
				PhoneNumber* deletenext = deletetemp->next;
				deletetemp->next = nullptr;
				delete deletetemp;
...
int PhoneBook::mainGUI(void)
{
    ...
    for(int i=3;i<10;i++){
        ...
        for(int i=11;i<19;i++){
            for(int j=1;j<16+i-20;j++)
            {
                	SetCursorPosition(j, i);
                    std::cout << " ";
            }
            for(int j=55;j>40-i+20;j--)
            {
                	SetCursorPosition(j, i);
                    std::cout << " ";
            }
...
void PhoneBook::deletenumberGUI(int funCode)
{
	...
	while (1) {
		...
		if (isPhoneNum(information))
		{
			...
			if (searchHashList(3, key2,nosense, information))
			{
		...
		else
		{
			...
			if (searchHashList(2, key1, information, nosense))
			{
				SetCursorPosition(15, 7);
				cout << "删除成功" << endl;
			}
...
bool PhoneBook::searchHashList(int functioncode, const int key,string name,string number)
{
	int findflag = 0;
	...
	string numberview=number;
	string tempnameview=tempname->name;
	string tempnumberview=tempnumber->number;
	#endif // DEBUG
	switch (functioncode)
	{
	case 0://根据名字只查询功能

		for (int count=0;count<lengthnameHash[key];count++)
		{
            #ifdef DEBUG
            nameview=name;
            numberview=number;
            tempnameview=tempname->name;
            tempnumberview=tempnumber->number;
            #endif // DEBUG
	...
	case 1://根据号码只查询功能
		...
		for (int count=0;count<lengthnumberHash[key];count++)
		{
